...
public:
  ...
  sc_uint<13> ADDR;
  sc_uint<2> BA;
  sc_uint<16> DATA_OUTPUT; // for write
  ...
  friend void sc_trace(sc_trace_file *tf, const sdram_io_master &v,
...
#define MEMBER_COPY_SDRAM_IO_MASTER(s, d)                                      \
  do {                                                                         \
    s.CLK = d.CLK;                                                             \
    s.CKE = d.CKE;                                                             \
    s.CS = d.CS;                                                               \
...
public:
  // Members
  sc_uint<16> DATA_INPUT; // for read

  // Construction
  sdram_io_slave() { init(); }

  void init(void) { DATA_INPUT = 0; }
  ...
  friend void sc_trace(sc_trace_file *tf, const sdram_io_slave &v,
                       const std::string &path) {
...
#define MEMBER_COPY_SDRAM_IO_SLAVE(s, d)                                       \
  do {                                                                         \
